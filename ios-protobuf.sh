...
CC=clang
CFLAGS="-DNDEBUG -g -O0 -pipe -fPIC -fcxx-exceptions -fembed-bitcode"
CXX=clang
CXXFLAGS="${CFLAGS} -std=c++11 -stdlib=libc++ -fembed-bitcode"
LDFLAGS="-stdlib=libc++"
LIBS="-lc++ -lc++abi"
echo "$(tput setaf 2)"
echo "####################################"
...
TARGET_PATH=/tmp/protobuf-${PB_VERSION}/src/.libs/libprotobuf.a
echo "$(tput setaf 2)"
echo "###############################################################"
...
X86_64_MAC_PROTOBUF_STRIP=x86_64/lib/libprotobuf-strip.a
X86_64_MAC_PROTOBUF_LITE=x86_64/lib/libprotobuf-lite.a
mkdir -p ${PLATFORM_DIR}/x86_64/lib
cp ${TARGET_PATH} ${PLATFORM_DIR}/${X86_64_IOS_SIM_PROTOBUF}
else
X86_64_MAC_PROTOBUF=
...
X86_64_IOS_SIM_PROTOBUF_STRIP=x86_64_ios/lib/libprotobuf-strip.a
X86_64_IOS_SIM_PROTOBUF_LITE=x86_64_ios/lib/libprotobuf-lite.a
mkdir -p ${PLATFORM_DIR}/x86_64_ios/lib
cp ${TARGET_PATH} ${PLATFORM_DIR}/${X86_64_IOS_SIM_PROTOBUF}
else
...
I386_IOS_SIM_PROTOBUF_STRIP=i386/lib/libprotobuf-strip.a
I386_IOS_SIM_PROTOBUF_LITE=i386/lib/libprotobuf-lite.a
mkdir -p ${PLATFORM_DIR}/i386/lib
cp ${TARGET_PATH} ${PLATFORM_DIR}/${I386_IOS_SIM_PROTOBUF}
else
...
ARMV7_IPHONE_PROTOBUF_STRIP=armv7/lib/libprotobuf-strip.a 
ARMV7_IPHONE_PROTOBUF_LITE=armv7/lib/libprotobuf-lite.a 
mkdir -p ${PLATFORM_DIR}/armv7/lib
cp ${TARGET_PATH} ${PLATFORM_DIR}/${ARMV7_IPHONE_PROTOBUF}
else
...
ARMV7S_IPHONE_PROTOBUF_STRIP=armv7s/lib/libprotobuf-strip.a 
ARMV7S_IPHONE_PROTOBUF_LITE=armv7s/lib/libprotobuf-lite.a 
mkdir -p ${PLATFORM_DIR}/armv7s/lib
cp ${TARGET_PATH} ${PLATFORM_DIR}/${ARMV7S_IPHONE_PROTOBUF}
else
...
(
    cd ${PREFIX}/platform
    mkdir universal
    # libprotobuf.a suprisingly big, so try strip
    strip -S -x -o ${ARM64_IPHONE_PROTOBUF_STRIP} -r ${ARM64_IPHONE_PROTOBUF}
    strip -S -x -o ${ARMV7_IPHONE_PROTOBUF_STRIP} -r ${ARMV7_IPHONE_PROTOBUF}
    strip -S -x -o ${X86_64_IOS_SIM_PROTOBUF_STRIP} -r ${X86_64_IOS_SIM_PROTOBUF}
    lipo ${ARM64_IPHONE_PROTOBUF_STRIP} ${ARMV7_IPHONE_PROTOBUF_STRIP} ${X86_64_IOS_SIM_PROTOBUF_STRIP} -create -output universal/libprotobuf-strip.a
